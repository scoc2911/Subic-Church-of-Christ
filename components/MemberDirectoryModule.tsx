...
export function MemberDirectoryModule({
  ...
  const getDynamicFontSizeStyle = (text: string, baseSizePt: number = 7.5) => {
    ...
    const len = text.length;
    let size = baseSizePt;
    if (len > 25) size = baseSizePt - 2.0;    // e.g. 5.5pt
    else if (len > 18) size = baseSizePt - 1.0; // e.g. 6.5pt
    else if (len > 12) size = baseSizePt - 0.5; // e.g. 7.0pt
        ...
        <div id="qr-modal" className="fixed inset-0 bg-black/75 backdrop-blur-xs z-50 flex items-center justify-center p-4 animate-fade-in overflow-y-auto">
          <div className="bg-slate-900 rounded-3xl max-w-sm w-full border border-slate-800 shadow-2xl overflow-hidden flex flex-col transform transition-all duration-300 scale-100 font-sans text-xs">
                  ...
                  <div className="p-4 flex-grow flex flex-col items-center justify-between relative bg-white h-[324px]">
                    ...
                    <div className="w-full grid grid-cols-2 gap-1.5 border-t border-slate-100 pt-2 text-left">
                      <div className="overflow-visible min-w-0">
                        ...
                        <p className={`font-extrabold text-slate-700 break-words mt-0.5 ${getDynamicFontSizeClass(selectedMemberForQr.ministry || "General Assembly")}`}>
                      ...
                      <div className="overflow-visible min-w-0">
                        ...
                        <p className={`font-extrabold text-slate-700 break-words mt-0.5 ${getDynamicFontSizeClass(selectedMemberForQr.network || "SCOC Network")}`}>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="bg-white p-2 rounded-xl border border-slate-150 space-y-1 text-left">
                        <div className="flex justify-between border-b border-dashed border-slate-100 pb-0.5">
                          ...
                          <span className="text-[7.5px] text-slate-800 font-bold font-mono tracking-tight">{selectedMemberForQr.membershipId || "—"}</span>
                        </div>
                        <div className="flex justify-between items-start border-b border-dashed border-slate-100 pb-0.5 gap-2">
                          ...
                          <span className={`text-slate-800 font-extrabold text-right break-words overflow-visible ${getDynamicFontSizeClassBack(selectedMemberForQr.ministry || "GENERAL ASSEMBLY")}`}>
                            {selectedMemberForQr.ministry || "GENERAL ASSEMBLY"}
                          </span>
                        </div>
                        <div className="flex justify-between items-start border-b border-dashed border-slate-100 pb-0.5 gap-2">
                          <span className="text-[6.5px] text-slate-400 font-extrabold uppercase shrink-0">NETWORK CLUSTER</span>
                          <span className={`text-slate-800 font-extrabold text-right break-words overflow-visible ${getDynamicFontSizeClassBack(selectedMemberForQr.network || "SCOC NETWORK")}`}>
                            {selectedMemberForQr.network || "SCOC NETWORK"}
                          </span>
                        </div>
                        {selectedMemberForQr.contactNumber && (
                          <div className="flex justify-between border-b border-dashed border-slate-100 pb-0.5">
                            <span className="text-[6.5px] text-slate-400 font-extrabold uppercase">CONTACT NO</span>
                            <span className="text-[7.5px] text-slate-800 font-bold font-mono">{selectedMemberForQr.contactNumber}</span>
                          </div>
                        )}
                        {selectedMemberForQr.address && (
                          <div className="flex flex-col text-left">
                            <span className="text-[6px] text-slate-400 font-extrabold uppercase mb-0.5">RESIDENTIAL ADDRESS</span>
                            <span className="text-[7px] text-slate-700 font-bold leading-tight line-clamp-2 uppercase">{selectedMemberForQr.address}</span>
                          </div>
                        )}
                      </div>
                        ...
                        <div className="grid grid-cols-2 gap-1 pt-0.5">
                          <div className="overflow-hidden">
                            ...
                            <p className="text-[7.5px] text-slate-800 font-extrabold truncate uppercase">
                              {selectedMemberForQr.emergencyContactPerson || "CHURCH OFFICE"}
                            </p>
                          </div>
                          <div className="overflow-hidden">
                            ...
                            <p className="text-[7.5px] text-slate-800 font-bold font-mono truncate">
                              {selectedMemberForQr.emergencyContactNumber || "0917-123-4567"}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-1 text-left bg-white p-1.5 rounded-xl border border-slate-150 items-center">
                        <div className="col-span-2 space-y-0.5">
                          <div className="leading-none">
                            ...
                            <span className="text-[7.5px] text-slate-800 font-bold font-mono block">{selectedMemberForQr.dateIssued || (selectedMemberForQr.createdAt ? selectedMemberForQr.createdAt.split('T')[0] : "2026-06-08")}</span>
                          </div>
                          <div className="leading-none">
                            <span className="text-[5.5px] text-slate-400 font-extrabold uppercase">EXPIRATION</span>
                            <span className="text-[7.5px] text-slate-800 font-extrabold uppercase tracking-tight block">{selectedMemberForQr.expirationDate || "LIFETIME"}</span>
                          </div>
                        </div>
            ...
            <div className="bg-slate-1000 border-t border-slate-850 px-5 py-4 flex items-center justify-end gap-2 text-xs">
                          ...
                          <head>
                            ...
                            <style>
                              ...
                              * {
                              ...
                              @page {
                                size: landscape; /* Fits standard paper types beautifully in landscape */
                                margin: 0.5in;
                              }
                              ...
                              .id-card {
                                ...
                                height: 3.375in;
                                background: #ffffff;
                                border-radius: 0.125in; /* Standard CR80 1/8 inch corner radius */
                                overflow: hidden;
                              ...
                              .card-header {
                                ...
                                justify-content: center;
                                gap: 0.06in;
                                border-bottom: 2px solid #00bae1;
                                height: 0.5in;
                              }
                              .header-logo {
                                width: 0.28in;
                              ...
                              .card-body {
                                padding: 0.12in 0.1in 0.08in 0.1in;
                                flex-grow: 1;
                                display: flex;
                                flex-direction: column;
                                align-items: center;
                              ...
                              /* BACK SIDE CONTAINER */
                              .card-body-back {
                                padding: 0.1in 0.1in 0.08in 0.1in;
                                flex-grow: 1;
                                display: flex;
                                flex-direction: column;
                                justify-content: space-between;
                                ...
                                <div class="card-body">
                                  ...
                                  <div class="info-grid">
                                    <div class="info-item" style="overflow: visible;">
                                      ...
                                      <span class="info-value" style="white-space: normal; word-break: break-word; text-overflow: clip; overflow: visible; ${getDynamicFontSizeStyle(selectedMemberForQr.ministry || "General Assembly", 7.5)}">${selectedMemberForQr.ministry || "General Assembly"}</span>
                                    </div>
                                    <div class="info-item" style="overflow: visible;">
                                      <span class="info-label">NETWORK CLUSTER</span>
                                      <span class="info-value" style="white-space: normal; word-break: break-word; text-overflow: clip; overflow: visible; ${getDynamicFontSizeStyle(selectedMemberForQr.network || "SCOC Network", 7.5)}">${selectedMemberForQr.network || "SCOC Network"}</span>
                                    </div>
                                  </div>
                                  ...
                                  <div class="member-id font-mono">MEMBER ID: ${selectedMemberForQr.membershipId || "—"}</div>
                                </div>
                                ...
                                <div class="card-body-back">
                                  <div class="specs-box">
                                    <div class="row-spec">
                                      ...
                                      <span class="val-spec font-mono">${selectedMemberForQr.membershipId || "—"}</span>
                                    </div>
                                    <div class="row-spec" style="align-items: flex-start; gap: 0.05in;">
                                      ...
                                      <span class="val-spec" style="white-space: normal; word-break: break-word; max-width: none; text-align: right; text-overflow: clip; overflow: visible; ${getDynamicFontSizeStyle(selectedMemberForQr.ministry || "GENERAL ASSEMBLY", 6.5)}">${selectedMemberForQr.ministry || "GENERAL ASSEMBLY"}</span>
                                    </div>
                                    <div class="row-spec" style="align-items: flex-start; gap: 0.05in;">
                                      <span class="label-spec" style="flex-shrink: 0; line-height: 1.2;">NETWORK CLUSTER</span>
                                      <span class="val-spec" style="white-space: normal; word-break: break-word; max-width: none; text-align: right; text-overflow: clip; overflow: visible; ${getDynamicFontSizeStyle(selectedMemberForQr.network || "SCOC NETWORK", 6.5)}">${selectedMemberForQr.network || "SCOC NETWORK"}</span>
                                    </div>
                                    ${selectedMemberForQr.contactNumber ? `
                                    <div class="row-spec">
                                      <span class="label-spec">CONTACT NO</span>
                                      <span class="val-spec font-mono">${selectedMemberForQr.contactNumber}</span>
                                    </div>
                                    ` : ""}
                                    ${selectedMemberForQr.address ? `
                                    <div class="row-spec flex-col">
                                      <span class="label-spec">RESIDENTIAL ADDRESS</span>
                                      <span class="val-spec text-small line-clamp">${selectedMemberForQr.address}</span>
                                    </div>
                                    ` : ""}
                                  </div>
                                  ...
                                  <div class="emergency-box">
                                    ...
                                    <div class="emergency-grid">
                                      <div class="emergency-col">
                                        ...
                                        <span class="emergency-val">${selectedMemberForQr.emergencyContactPerson || "CHURCH OFFICE"}</span>
                                      </div>
                                      <div class="emergency-col text-right">
                                        <span class="emergency-label">CONTACT NO</span>
                                        <span class="emergency-val font-mono">${selectedMemberForQr.emergencyContactNumber || "0917-123-4567"}</span>
                                      </div>
                                    </div>
                                  </div>
                                  ...
                                  <div class="validity-qr-row">
                                    <div class="validity-block">
                                      <div>
                                        ...
                                        <span class="val-spec text-mono-small font-mono">${selectedMemberForQr.dateIssued || (selectedMemberForQr.createdAt ? selectedMemberForQr.createdAt.split('T')[0] : "2026-06-08")}</span>
                                      </div>
                                      <div style="margin-top: 4px;">
                                        <span class="label-spec text-micro">EXPIRATION</span>
                                        <span class="val-spec text-mono-small">${selectedMemberForQr.expirationDate || "LIFETIME"}</span>
                                      </div>
                                    </div>
                                    <div class="mini-qr">
                                  ...
                                  <div class="reminders-box">
                                    <div>• This card is the property of Subic Church of Christ.</div>
                                    <div>• If found, please surrender to church administrative offices.</div>
                                    <div>• Official Website: <b>www.subicchurchofchrist.org</b></div>
                                  </div>
                                </div>
                                <div class="card-footer">
                                  ZAMBALES, PH • SUBICCHURCHOFCHRIST@GMAIL.COM
                                </div>
                              </div>
              ...
              >
                <Printer className="w-3.5 h-3.5 text-gray-450" />
                Print Card Pass
              </button>
